...
function loadItemsFromLocalStorage() {
  ...
  var template =
    '<div class="b-page-test-switch__item b-page border-radius" index="{1}">{0}</div>'
  ...
  for (var i = 0; localStorage['ExpertSys' + i]; i++) {
    target.html(
      target.html() +
        template
          .replace('{1}', 'ExpertSys' + i)
          .replace('{0}', JSON.parse(localStorage['ExpertSys' + i]).title)
    )
  }
}
...
function init() {
  ...
  $('#start-test').bind('click keypress', function() {
    if (
      ...
      $('.b-page-test-switch__selected').length > 0
    ) {
      testSystem = loadFromLocalStorage(
        $('.b-page-test-switch__selected').attr('index')
      )
    } else {
      testSystem = new Test()
  ...
  $('.b-page-test-switch__item').live('click keypress', function() {
    ...
    $('.b-page-test-switch__selected').removeClass(
      'b-page-test-switch__selected'
    )
  ...
  //   init answer buttons actions
  $('.answer-block .b-button').click(function(e) {
    var value = e.target.attributes['data-value'].value
    $('#current-answer').val(value)
    $('#complete-answer').click()
  })
...
function sortItems(a, b) {
  if (a.points > b.points || (a.points == b.points && a.title > b.title))
    return 1
...
function sortQuestion(a, b) {
  ...
  for (var i = 0; i < a.items.length; i++) {
    aPoints +=
      a.items[i].questionPoints[a.index].min +
      a.items[i].questionPoints[a.index].max +
      a.items[i].points
  }
  for (var i = 0; i < b.items.length; i++) {
    bPoints +=
      b.items[i].questionPoints[b.index].min +
      b.items[i].questionPoints[b.index].max +
      b.items[i].points
  }
...
Test.prototype.printData = function() {
  ...
  var template =
    '<div title="{0}: {1}" class="b-page-test-items__item border-radius"><span class="b-page-test-items__item-title">{0}</span><span class="b-page-test-items__item-percent">{1}</span></div>'
...
Test.prototype.nextStep = function() {
  ...
  if (this.questions.length == 0 || this.complete) {
    $('#current-question').html(
      'Ознакомьтесь с решением системы. Вопросы закончены.'
    )
    $('#complete-answer')
      .add('#current-answer')
      .addClass('hide')
    $('.answer-block').addClass('hide')
    return
  }
...
Test.prototype.processAnswer = function(ans) {]
  for (var i = 0; i < this.items.length; i++) {
    ...
    if (point) {
      var up =
        (((2 * point.max - 1) * ans) / 100 + 1 - point.max) *
        this.items[i].points
      var down =
        (((2 * point.max - 1) * ans) / 100 + 1 - point.max) *
          this.items[i].points +
        (((2 * point.min - 1) * ans) / 100 + 1 - point.min) *
          (1 - this.items[i].points)
...
Test.prototype.parseData = function(data) {
  try {
    ...
    var passEmptyStrings = function() {
      while (position < items.length && items[position] == '') {
        position++
      }
    }
